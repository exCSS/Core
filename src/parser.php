		...
		public function parse($ex_file, $s_file)
			{
				...
				if(isset($ex['ExtendedCSS']))
				{
					while($cur = current($ex['ExtendedCSS']))
					{
						...
						$VariableStore->set_variable($var,$var_value);
						next($ex['ExtendedCSS']);
					}
				}
				...
				foreach($VariableStore->foreach_var() as $key)
				{
					$css = str_replace("$".$key, $VariableStore->get_variable($key), $css);
				}
